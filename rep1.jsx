// ===========================================================================
// 1. LES COMPOSANTS EXTERNES (Carrousel + Chat IA)
// ===========================================================================

// --- COMPOSANT CARROUSEL (Balayable & Premium) ---
function ImageSlider({ images }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '180px', backgroundColor: '#2a2a2a', borderRadius: '12px', overflow: 'hidden', userSelect: 'none' }} 
      ...
      <button onClick={handlePrev} style={{ position: 'absolute', left: '8px', top: '50%', transform: 'translateY(-50%)', backgroundColor: 'rgba(0,0,0,0.5)', border: 'none', color: 'white', padding: '8px 12px', borderRadius: '50%', cursor: 'pointer' }}>❮</button>
      <button onClick={handleNext} style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', backgroundColor: 'rgba(0,0,0,0.5)', border: 'none', color: 'white', padding: '8px 12px', borderRadius: '50%', cursor: 'pointer' }}>❯</button>
...
function ChatWidget({ settings, isChatOpen, setIsChatOpen, chatMessages, setChatMessages, chatInput, setChatInput, isAiLoading, setIsAiLoading }) {
  ...
  const handleSendMessage = async () => {
    ...
    // Mode Simulation (si pas de clé API Open AI)
    if (!settings.openAiApiKey) {
      setTimeout(() => {
        setChatMessages(prev => [...prev, { 
          role: 'assistant', 
          content: "🤖 (Assistant virtuel) : Je suis l'agent IA de TractoRent. Je suis configuré pour répondre à vos questions. Pour débloquer mon intelligence artificielle avancée, veuillez entrer votre clé API dans le Panneau Administrateur." 
        }]);
    ...
    // Mode Réel (Appel à l'API OpenAI)
    try {
      const response = await fetch(settings.iaAgentApiUrl, {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: settings.iaAgentSystemMessage },
...
function App() {
  ...
  // Remplacez ces 3 variables par vos véritables clés EmailJS
  ...
  // Données par défaut des 8 tracteurs Premium (Sera remplacé par VOS données lors de la fusion)
  const defaultTractors = [
    { id: 1, name: "Steyr 6150 Profi", model: "Stage V", pricePerDay: 280, category: "Premium", images: ["https://images.unsplash.com/photo-1535201970725-0941f77542c2?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1586528116311-ad8edd3a6e40?w=600&h=400&fit=crop"] },
    { id: 2, name: "Lamborghini Spark", model: "125 VRT", pricePerDay: 350, category: "Premium", images: ["https://images.unsplash.com/photo-1590947134131-44872e050e5e?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1590605185932-21c251e85f2a?w=600&h=400&fit=crop"] },
    { id: 3, name: "Fendt 728 Vario", model: "Gen7 Profi+", pricePerDay: 420, category: "Premium", images: ["https://images.unsplash.com/photo-1506435287758-bb336e5ef418?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1625246333195-58dba73b5158?w=600&h=400&fit=crop"] },
    { id: 4, name: "Claas Arion 570", model: "CMATIC CEBIS", pricePerDay: 240, category: "Standard", images: ["https://images.unsplash.com/photo-1586528116311-ad8edd3a6e40?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1535201970725-0941f77542c2?w=600&h=400&fit=crop"] },
    { id: 5, name: "New Holland T6.180", model: "Auto Command", pricePerDay: 190, category: "Standard", images: ["https://images.unsplash.com/photo-1590605185932-21c251e85f2a?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1590947134131-44872e050e5e?w=600&h=400&fit=crop"] },
    { id: 6, name: "John Deere 6210R", model: "Premium Edition", pricePerDay: 390, category: "Premium", images: ["https://images.unsplash.com/photo-1625246333195-58dba73b5158?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1506435287758-bb336e5ef418?w=600&h=400&fit=crop"] },
    { id: 7, name: "Kubota M7-171", model: "Premium KVT", pricePerDay: 160, category: "Standard", images: ["https://images.unsplash.com/photo-1535201970725-0941f77542c2?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1586528116311-ad8edd3a6e40?w=600&h=400&fit=crop"] },
    { id: 8, name: "Case IH Puma 240", model: "CVXDrive", pricePerDay: 310, category: "Premium", images: ["https://images.unsplash.com/photo-1590947134131-44872e050e5e?w=600&h=400&fit=crop","https://images.unsplash.com/photo-1590605185932-21c251e85f2a?w=600&h=400&fit=crop"] },
  ];
  ...
  const submitBooking = (e) => {
    ...
    // 👉 [VOS API EXISTANTES ICI] : Si vous avez déjà des appels API Stripe ou autres pour le paiement, vous pouvez les insérer ici.
  ...
  const updateSiteSettings = (e) => {
    ...
    setSettings({
      ...settings, siteName: data.get('siteName'), adminEmail: data.get('adminEmail'), primaryColor: data.get('primaryColor'), secondaryColor: data.get('secondaryColor'), bgColor: data.get('bgColor'), iaAgentEnabled: data.get('iaAgentEnabled') === 'on', iaAgentSystemMessage: data.get('iaAgentSystemMessage'), openAiApiKey: data.get('openAiApiKey')
    });
    setMessage("✅ Configuration du site mise à jour !");
    setTimeout(() => setMessage(''), 3000);
  };

  // --- RENDU VISUEL DU SITE ---
  ...
  return (
    <div style={{ backgroundColor: theme.bg, minHeight: '100vh', fontFamily: settings.fontFamily, color: '#fff', paddingBottom: '30px' }}>
      ...
      {/* Notifications */}
      {message && (
        <div style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: theme.primary, color: '#fff', padding: '15px 25px', borderRadius: '10px', zIndex: 9999, boxShadow: '0 8px 25px rgba(0,0,0,0.6)', fontWeight: '500', maxWidth: '90%', textAlign: 'center', transition: '0.3s' }}>
          {message}
        </div>
      )}
      ...
      <nav style={{ backgroundColor: theme.primary, padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', cursor: 'pointer' }} onClick={() => setIsAdminMode(!isAdminMode)}>
          <span style={{ fontSize: '20px', color: '#fff', transition: '0.2s' }}>⚙️</span>
        </div>
      </nav>
      ...
      {isAdminMode && (
        <div style={{ margin: '20px', backgroundColor: '#1f1f1f', padding: '20px', borderRadius: '16px', border: `2px solid ${theme.secondary}` }}>
          <h3 style={{ marginBottom: '15px', color: theme.secondary, display: 'flex', alignItems: 'center', gap: '10px' }}>🔐 Tableau de bord Administrateur</h3>
          {adminPassword !== 'admin123' ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '25px' }}>
              ...
              {/* 3. Liste et Actions sur les tracteurs */}
              <div style={{ backgroundColor: '#2a2a2a', padding: '15px', borderRadius: '10px' }}>
      ...
      {/* --- CATALOGUE CLIENT (Affichage des tracteurs) --- */}
      ...
      {reservations.length > 0 && (
        <div style={{ margin: '20px 15px', backgroundColor: '#1f1f1f', borderRadius: '12px', padding: '20px' }}>
          <h3 style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>📋 Mes réservations</h3>
          {reservations.map(res => (
            <div key={res.id} style={{ borderBottom: '1px solid #333', padding: '15px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
              ...
              {res.status === 'Paiement confirmé - Réservation réussie' && (
                <button onClick={() => { alert("Votre reçu PDF va être généré automatiquement.") }} style={{ backgroundColor: '#fff', color: '#121212', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>

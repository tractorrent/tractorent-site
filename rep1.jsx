...
function App() {
  ...
  return (
    <div style={{ backgroundColor: theme.bg, minHeight: '100vh', fontFamily: settings.fontFamily, color: '#fff', paddingBottom: '30px' }}>
        ...
        <div style={{ margin: '20px', backgroundColor: '#1f1f1f', padding: '20px', borderRadius: '16px', border: `2px solid ${theme.secondary}` }}>
          ...
          {adminPassword !== 'admin123' ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '25px' }}>
              ...
              {/* --- SECTION BLOG DANS L'ADMIN --- */}
              <div style={{ backgroundColor: '#2a2a2a', padding: '15px', borderRadius: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <h4 style={{ color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    📖 Gestion du Blog IA Autonome
                  </h4>
                  <button onClick={fetchBlogPosts} disabled={isBlogLoading} style={{ backgroundColor: theme.secondary, border: 'none', padding: '6px 15px', borderRadius: '4px', fontWeight: 'bold', color: '#121212', cursor: 'pointer', fontSize: '13px' }}>
                    {isBlogLoading ? "Chargement..." : "🔄 Rafraîchir le flux"}
                  </button>
                </div>

                {isBlogLoading ? (
                  <p style={{ color: '#888', fontSize: '13px' }}>Récupération des articles générés par l'IA...</p>
                ) : blogPosts.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '250px', overflowY: 'auto', paddingRight: '5px' }}>
                    {blogPosts.map((post, index) => (
                      <div key={index} style={{ backgroundColor: '#333', padding: '12px 15px', borderRadius: '6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                        <div>
                          <div style={{ fontWeight: 'bold', fontSize: '15px', color: '#fff' }}>{post.title || "Article sans titre"}</div>
                          <div style={{ fontSize: '12px', color: '#aaa' }}>Publié le {post.date || "Date inconnue"} • {post.category || "Non catégorisé"}</div>
                        </div>
                        <div>
                          <a href={post.link || "#"} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: '#555', color: '#fff', textDecoration: 'none', padding: '5px 12px', borderRadius: '4px', fontSize: '13px', display: 'inline-block' }}>
                            Voir l'article
                          </a>
                        </div>
                      </div>
                  ...
                  <div style={{ backgroundColor: '#333', padding: '15px', borderRadius: '6px', textAlign: 'center', color: '#888', fontSize: '13px' }}>
                    ⚡ Aucun article trouvé. <br/>
                    (Assurez-vous que votre second dépôt génère bien un fichier `feed.json` à l'adresse indiquée dans le code).
                  </div>
                )}
              </div>
      ...
      {/* --- SECTION BLOG IA PUBLIQUE --- */}
      <div style={{ margin: '20px 15px', backgroundColor: '#1f1f1f', borderRadius: '16px', padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '8px', color: '#fff' }}>
            🤖 Articles générés par notre IA
          </h3>
          <a href="https://tractorrent.github.io/tractorrent-blog-cron/" target="_blank" rel="noopener noreferrer" 
             style={{ backgroundColor: theme.primary, color: '#fff', padding: '8px 15px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px' }}>
            Voir tout le blog →
          </a>
        </div>
        ...
        {blogPosts.length > 0 ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '15px' }}>
            {blogPosts.slice(0, 3).map((post, index) => (
              <div key={index} style={{ backgroundColor: '#333', padding: '15px', borderRadius: '12px' }}>
                <div style={{ fontWeight: 'bold', fontSize: '16px', color: '#fff' }}>{post.title || "Titre de l'article"}</div>
                <div style={{ fontSize: '12px', color: '#aaa', marginTop: '5px' }}>Publié le {post.date || "Aujourd'hui"} par l'IA</div>
                <p style={{ fontSize: '13px', color: '#ddd', marginTop: '8px', lineHeight: '1.4' }}>{post.excerpt || "Cliquez sur lire la suite pour découvrir l'article complet généré automatiquement par notre intelligence artificielle."}</p>
                <a href={post.link || "#"} target="_blank" rel="noopener noreferrer" style={{ color: theme.secondary, fontSize: '12px', textDecoration: 'none', fontWeight: 'bold' }}>Lire la suite →</a>
              </div>
            ))}
          </div>
        ) : (
          <div style={{ textAlign: 'center', color: '#888', padding: '20px', backgroundColor: '#333', borderRadius: '12px' }}>
            <p style={{ fontSize: '14px' }}>📝 En attente du premier article généré par l'IA...</p>
            <p style={{ fontSize: '12px' }}>Votre cron alimentera cette section automatiquement.</p>
          </div>
        )}
      </div>
